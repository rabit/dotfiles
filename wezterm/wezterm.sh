...
# This function emits an sequence to inform the terminal
# of the user variables.
__wezterm_user_vars() {
  local OS="$(lsb_release -s -i)"
  printf "\033]1337;SetUserVar=%s=%s\007" os `echo -n ${OS} | base64`
}
...
if [[ -z "${WEZTERM_SHELL_SKIP_CWD}" ]] ; then
  if [[ -n "$BLE_VERSION" ]]; then
    ...
    blehook PRECMD+=__wezterm_user_vars
  else
